...
| **About** | Zeigt installierte Firmware-Version, Hersteller, PolyForm Noncommercial 1.0.0, Chipmodell, Flashgröße, aktiven OTA-Slot, System-NVS, Profil-NVS und Verlaufspartition sowie den Quellcode-/Projektlink. Zusätzlich erscheinen die zuletzt auf GitHub gefundene Firmware, Prüfzeit und Status; „Jetzt prüfen“ startet eine manuelle HTTPS-Abfrage. Lizenz- und Haftungshinweise sowie Web-Debug bleiben auf derselben Seite. |
...
Optional werden ausgewählte Ereignisse und ein Tagesbericht an Pushover gesendet. In einem Ausklappbereich lassen sich Start/Neustart, Modbus-Ausfall/Wiederherstellung, Netzausfall/Netzwiederkehr und neue Firmwareversionen getrennt aktivieren. Für den einmaligen Bericht nach Sonnenuntergang sind PV-Tagesertrag 13001 und absoluter Batteriestand 10743 einzeln auswählbar. Dafür werden der Application/API-Token einer eigenen Pushover-Anwendung und der persönliche User-/Group-Key hinterlegt; ein Gerätebezeichner kann den Empfang auf ein Gerät begrenzen. Die beiden Schlüssel werden nach dem Speichern nicht wieder in das Webformular eingesetzt. Leere Schlüsselfelder behalten vorhandene Werte, das Löschfeld entfernt sie ausdrücklich.

Die Ausfallverzögerung unterdrückt kurze Modbus- und Netzstatusstörungen. Bevorzugt stammt der Netzstatus aus dem Sungrow-Herstellerregister 13030, das im nullbasierten Sketch als Adresse 13029 gelesen wird: `0x55` steht für Netzbetrieb, `0xAA` für Inselbetrieb/Netzausfall. Ist dieser Wert nicht verfügbar, liest die Firmware den 32-Bit-Netzfehler aus den Herstellerregistern 13052–13053, nullbasiert ab Adresse 13051. Ausschließlich Bit 8 „Grid Power Outage“ gilt darin als ausdrücklicher Netzausfall; andere Netzfehlerbits lösen diese Meldung nicht aus. Eine Wiederkehr wird erst bestätigt, wenn Bit 8 gelöscht und Herstellerregister 5036 „Grid frequency“ (nullbasiert 5035, Faktor 0,01 Hz) wieder zwischen 45 und 65 Hz liegt. Alle drei Blöcke sind optional, damit abweichende Firmwarestände die übrige Abfrage nicht blockieren. Nach erfolgreicher Ausfallmeldung entsteht keine wiederholte Dauermeldung; nach Wiederherstellung folgt genau ein weiterer Zustandswechsel. Das Datum eines erfolgreich gesendeten Tagesberichts bleibt im NVS gespeichert und verhindert eine Doppelmeldung nach Neustart. Scheitert die HTTPS-Übertragung, bestimmt der Wiederholungsabstand den nächsten Versuch. Der Testknopf prüft neue Eingaben ohne vorheriges Speichern. Der offizielle Pushover-Endpunkt wird per HTTPS mit Zertifikatsprüfung angesprochen.

Die Firmwareprüfung ruft höchstens einmal täglich das neueste stabile Release des öffentlichen GitHub-Projekts per HTTPS ab. Dafür wird kein GitHub-Token gespeichert. Eine numerisch höhere Major-, Minor- oder Patchversion erzeugt genau eine Pushover-Nachricht; die gemeldete Version wird zur Entdoppelung im NVS abgelegt. Fehler bei GitHub oder der Internetverbindung führen nur zu einem späteren Wiederholungsversuch. Automatisches Herunterladen oder Installieren ist ausgeschlossen.

Bei vollständigem Strom- oder Internetausfall kann der ESP32 naturgemäß keine Sofortmeldung übertragen. Auch bei nahtlosem Inselbetrieb müssen ESP32, Router und Internetzugang weiter versorgt werden; die Netzwerkkomponenten müssen dafür am Ersatzstrom hängen. Ein Inselwechselrichter kann lokal weiterhin 50 Hz erzeugen. Deshalb ergänzt `Grid-side fault` Bit 8 die Frequenzprüfung. Bei einer kontrollierten Netztrennung muss geprüft werden, ob das Bit am jeweiligen Modell tatsächlich gesetzt wird und nach der Rückkehr wieder erlischt. Sobald Gerät, WLAN, Internet und NTP-Zeit wieder verfügbar sind, wird der Start/Neustart gemeldet. Die Funktion ist eine Komfortbenachrichtigung und keine zertifizierte Alarmanlage oder Netzschutzeinrichtung.
...
| Wechselrichter | 5035 | Netzfrequenz aus Herstellerregister 5036 in Hz; optionaler Rückfallwert für die Netzausfallerkennung |
...
| Wechselrichter (Input) | 13051 / 13052 | 32-Bit-Grid-side-fault; Bit 8 meldet „Grid Power Outage“. Nicht mit Holding 13051 zu verwechseln. |
